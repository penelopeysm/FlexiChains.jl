function _discquantiles_bands(data, quantiles, baseline, residual)
    ...
    n = length(data)
    residual && baseline === nothing && throw(ArgumentError("`residual=true` requires `baseline`"))
    baseline === nothing || length(baseline) == n || throw(ArgumentError(
...
function _plot_discquantiles!(
        ...
        baseline = nothing, residual = false,
        color = Makie.Cycled(1), direction::Symbol = :y, kwargs...,
    )
    qs = _discquantiles_bands(data, quantiles, baseline, residual)
    nq = size(qs, 1); n = size(qs, 2); n_bands = nq ÷ 2; median_idx = (nq + 1) ÷ 2
    ...
    p = nothing
    for i in 1:n_bands
        p = Makie.barplot!(ax, positions, qs[nq + 1 - i, :];
            fillto = qs[i, :], color = (base_color, _band_alpha(i, n_bands)),
            strokewidth = 0, direction = direction, width = 0.6, kwargs...)
    end
    median_p = if direction === :y
        Makie.scatter!(ax, positions, medians; color = base_color, marker = :hline, markersize = 16)
    else
        Makie.scatter!(ax, medians, positions; color = base_color, marker = :vline, markersize = 16)
    end
    p === nothing && (p = median_p)  # median-only case (n_bands == 0)
    if baseline !== nothing && !residual
        bl = collect(Float64, baseline)
        if direction === :y
            Makie.scatter!(ax, positions, bl; color = :black, marker = :xcross, markersize = 12)
        else
            Makie.scatter!(ax, bl, positions; color = :black, marker = :xcross, markersize = 12)
        end
    elseif residual
        if direction === :y
...
# ---- vertical (default) ----
function FC.Makie.discquantiles(chn::FC.FlexiChain, param; figure = (;), axis = (;), kwargs...)
    return _discquantiles_figure(chn, param, :y; figure = figure, axis = axis, kwargs...)
end
...
# ---- rotated / horizontal ----
function FC.Makie.discquantiles_vert(chn::FC.FlexiChain, param; figure = (;), axis = (;), kwargs...)
    return _discquantiles_figure(chn, param, :x; figure = figure, axis = axis, kwargs...)
end
