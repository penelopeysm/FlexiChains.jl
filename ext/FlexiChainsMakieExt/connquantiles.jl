...
function FC.Makie.connquantiles(
        chn::FC.FlexiChain, param, plot_xs = nothing;
    ...
    _, p = FC.Makie.connquantiles!(ax, chn, param, plot_xs; quantiles = quantiles, kwargs...)
    return Makie.FigureAxisPlot(fig, ax, p)
end

function FC.Makie.connquantiles!(
        ax::Makie.Axis, chn::FC.FlexiChain, param, plot_xs = nothing;
        quantiles = FC.PlotUtils.DEFAULT_QUANTILE_LEVELS,
    ...
    length(xs) == n || throw(ArgumentError(
    ...
    p = nothing
    for i in 1:n_bands
        p = Makie.band!(ax, xs, qs[i, :], qs[nq + 1 - i, :];
    ...
    elseif baseline !== nothing
        Makie.lines!(ax, xs, collect(Float64, baseline);
