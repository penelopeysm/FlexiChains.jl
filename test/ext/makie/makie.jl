...
# Reference tests as recommended by Julius Krumbiegel
...
function compare_images(
    a::AbstractMatrix{<:Union{Makie.RGB,Makie.RGBA}},
    b::AbstractMatrix{<:Union{Makie.RGB,Makie.RGBA}},
)
    ...
    range_dim1 =
        round.(
            Int, range(0, size(a, 1); length=ceil(Int, size(a, 1) / approx_tile_size_px))
        )
    range_dim2 =
        round.(
            Int, range(0, size(a, 2); length=ceil(Int, size(a, 2) / approx_tile_size_px))
        )

    boundary_iter(boundaries) = zip(boundaries[1:(end - 1)] .+ 1, boundaries[2:end])

    function _norm(rgb1::RGBf, rgb2::RGBf)
        return sqrt(sum(((rgb1.r - rgb2.r)^2, (rgb1.g - rgb2.g)^2, (rgb1.b - rgb2.b)^2)))
    end
    function _norm(rgba1::RGBAf, rgba2::RGBAf)
        return sqrt(
            sum((
                (rgba1.r - rgba2.r)^2,
                (rgba1.g - rgba2.g)^2,
    ...
    return maximum(
        Iterators.product(boundary_iter(range_dim1), boundary_iter(range_dim2))
    ) do ((mi1, ma1), (mi2, ma2))
